...
import { areaUnits, convert } from '@/lib/calculators/converters';

export default function AreaConverter() {
  const [value, setValue] = useState('1');
  const [fromUnit, setFromUnit] = useState('acres');
  const [toUnit, setToUnit] = useState('squareMeters');

  const unitOptions = Object.keys(areaUnits).map(key => ({
    value: key,
  ...
  const result = useMemo(() => {
    ...
    return convert(num, fromUnit, toUnit, areaUnits);
        ...
        <div className="mt-8 pt-6 border-t border-border">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {unitOptions.filter(u => u.value !== fromUnit).slice(0, 8).map(opt => {
              ...
              const converted = convert(num, fromUnit, opt.value, areaUnits);
              const displayVal = converted % 1 !== 0 ? converted.toFixed(6).replace(/\.?0+$/, '') : converted;
