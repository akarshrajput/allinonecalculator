...
import { lengthUnits, convert } from '@/lib/calculators/converters';
...
export default function LengthConverter() {
  ...
  const unitOptions = Object.keys(lengthUnits).map(key => ({
    value: key,
  ...
  const result = useMemo(() => {
    ...
    return convert(num, fromUnit, toUnit, lengthUnits);
  }, [value, fromUnit, toUnit]);

  // Handle bidirectional update
  const handleSwap = () => {
    setFromUnit(toUnit);
    setToUnit(fromUnit);
  };
        ...
        <div className="mt-8 pt-6 border-t border-border">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {unitOptions.filter(u => u.value !== fromUnit).slice(0, 8).map(opt => {
              ...
              const converted = convert(num, fromUnit, opt.value, lengthUnits);
